...
def user_stats(df):
    ...
    # Filter out invalid years (e.g., 0 or any other nonsensical year)
    valid_years = year[year > 0]

    # Compute statistics
    earliest_year = valid_years.min()
    most_recent_year = valid_years.max()
    most_common_year = valid_years.mode()[0]

    # Print results
    print(f'Earliest birth year is: {earliest_year}')
    print(f'Most recent birth year is: {most_recent_year}')
    print(f'Most common birth year is: {most_common_year}')
